...
Отвечает за создание главной страницы — каталога товаров с отображением счётчика. Содержит набор сеттеров, которые управляют DOM-элементами главной страницы.  
...
Принимает в конструктор экземпляр класса Api и включает методы, обеспечивающие взаимодействие с серверной частью сервиса.
...
Стандартная реализация брокера событий. Предоставляет возможность отправлять события и подписываться на них,
отслеживая происходящие в системе изменения. Класс применяется в презентере для обработки событий
и в различных слоях приложения для их генерации.
