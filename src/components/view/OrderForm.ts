...
export class OrderForm implements IOrderModel {
	...
	constructor(template: HTMLTemplateElement, private events: IEvents) {
		...
		) as HTMLButtonElement[]; // Получаем все кнопки оплаты
		this.submitButton = this.formElement.querySelector(
			'.order__button'
		)! as HTMLButtonElement; // Кнопка отправки
	...
	public setupEventListeners(): void {
		...
		this.paymentButtons.forEach((button) => {
			button.addEventListener('click', () => {
				...
				this.events.emit('order:paymentMethodSelected', {
					payment: button.name,
				}); // Отправляем событие с выбранным методом
			});
		});
		...
		this.formElement.addEventListener('input', (event: Event) => {
			...
			this.events.emit('order:addressFieldChanged', {
				field: fieldName,
				value: fieldValue,
			}); // Отправляем событие с изменениями
		...
		this.events.on('order:validate', (data: { isValid: boolean }) => {
	...
	public updateSubmitButton(): void {
		...
		const isValid = !!this.selectedPayment && address.length != 0;
		this.events.emit('order:validate', { isValid });
	}
